...
axes_init_x = 1.333333333333;
axes_init_y = 0.666666666666;
axes_init_z = 0.5;
axes_x = axes_init_x/m;
axes_y = axes_init_y/n;
axes_z = axes_init_z/p;
axes_init_x1 = 0.333333333;
axes_init_y1 = 0.666666666;
axes_init_z1 = 0.5;
...
global m n p
    ...
    impurity_c = 0.5/p;
    % Step 1: Move the line with coordinates [1, 2, 3] to the end
    for i = 1:length(lines)
        if i > 8 % Coordinates typically start from line 8
            coords = str2num(lines{i}); %#ok<ST2NM>
            if ~isempty(coords)
                % Compare with tolerance to check for [1, 2, 3] coordinate line
                if norm(coords - [impurity_a, impurity_b, impurity_c]) < tolerance
                    move_coords_line = lines{i}; % Save the [1, 2, 3] coordinate line
                    continue; % Skip output of the [1, 2, 3] coordinate line
                end
            end
        end
    ...
    % Add the [a, b, c] coordinate line to the end
    if ~isempty(move_coords_line)
        new_lines{end+1} = move_coords_line;
    else
        warning('No line with coordinates [1, 2, 3] found.');
    end
